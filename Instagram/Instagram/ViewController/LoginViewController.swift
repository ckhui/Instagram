...
class LoginViewController: UIViewController {
    ...
    func loginButtonTapped(button: UIButton)
    {
        ...
        {   // warn user that they need to enter username or password
            ...
            Twitter().warningPopUp(withTitle: "Error", withMessage: "email and name error", viewController: self)
            return
        }
        ...
        FIRAuth.auth()?.signIn(withEmail: username, password: password, completion: {(user, error) in
            ...
            if let authError = error {
                ...
                //TODO: password error
                Twitter().warningPopUp(withTitle: "Log in Error", withMessage: "password and email not match", viewController: self)
